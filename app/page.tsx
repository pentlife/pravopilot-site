...
const features = [
  ...
  ["🤖", "AI-подготовка по ситуации", "Предварительный ответ помогает собрать материалы и подготовить вопросы специалисту."],
  ["🔍", "AI-подготовка документа", "Бот помогает выделить значимые условия документа и зоны внимания для последующей проверки специалистом."],
];
...
const socialLinks = [
  ["Бот", site.botUrl, "M20.8 4.7 17.7 19.6c-.2.8-1.1 1-1.7.6l-4.8-3.6-2.3 2.2c-.5.5-1.3.2-1.1-.5l.9-3.8 7.1-6.4-8.8 5.5-3.8-1.2c-.8-.3-.8-1.4.1-1.7l16.9-6.5c.7-.3 1.4.3 1.2 1.1Z"],
  ["Поддержка", site.supportUrl, "M12 2a10 10 0 0 0-8.7 14.9L2 22l5.3-1.2A10 10 0 1 0 12 2Zm4.8 14.1c-.2.6-1.2 1.1-1.7 1.2-.5.1-1.1.2-3.4-.7-2.9-1.1-4.8-4-4.9-4.2-.1-.1-1.2-1.6-1.2-3.1s.8-2.2 1.1-2.5c.2-.3.6-.4.9-.4h.6c.2 0 .5 0 .7.5l.9 2.2c.1.3.1.5 0 .7l-.4.6c-.1.2-.3.4-.1.7.2.4.8 1.3 1.7 2.1 1.2 1 2.1 1.3 2.5 1.5.3.1.5.1.7-.1l1-1.2c.2-.3.5-.3.8-.2l2 .9c.3.2.6.3.6.5 0 .1 0 .7-.2 1.4Z"],
  ["ВКонтакте", "/#bot", "M3.4 7.2h3.2c.1 4.1 1.9 5.8 3.3 6.2V7.2h3v3.5c1.4-.2 2.9-1.8 3.4-3.5h3c-.4 2.1-2.2 3.7-3.5 4.4 1.3.6 3.4 2 4.2 4.9h-3.3c-.6-1.6-1.9-2.9-3.8-3.1v3.1h-.4c-6.4 0-9.8-4.4-10.1-9.3Z"],
];
...
{/* Modern Hero Section */}
      <section className="hero-modern">
        <div className="hero-modern-grid">
          <div className="hero-modern-content">
            ...
            <p className="lead">50+ бесплатных шаблонов документов, предварительная AI-подготовка материалов и структурированное резюме для юриста — всё в боте.</p>
          ...
          <div className="hero-modern-image">
            <img src="/images/hero-legal-assistant.png" alt="Иллюстрация: бот помогает структурировать документы для консультации" />
          </div>
        </div>
      </section>
      ...
      <section className="process" id="features"><div className="wrap"><div className="section-title"><div><div className="kicker">Возможности</div><h2>Три сценария в одном боте</h2></div><p>Сначала — бесплатные формы и AI-подготовка материалов. Затем, если вопрос сложный, структурированное резюме можно передать адвокату для профессиональной оценки.</p></div><div className="audiences">{features.map(([icon,title,text])=><article className="aud" key={title}><div className="mini">{icon}</div><h3>{title}</h3><p className="muted">{text}</p><a className="btn ghost" href={site.botUrl}>Открыть бота</a></article>)}</div></div></section>

      <section id="templates"><div className="wrap"><div className="section-title"><div><div className="kicker">Попробуйте бесплатно</div><h2>Бесплатные образцы документов в боте</h2></div><p>Откройте каталог, выберите типовую форму и скачайте текстовый файл. AI-подготовка и загрузка документов включаются только если вы сами выбираете расширенный режим.</p></div><div className="result-grid"><div className="result-card"><h3>Что доступно сразу</h3>{freeBotItems.map(x=><div className="check" key={x}>{x}</div>)}{templates.slice(0,5).map(x=><div className="check" key={x}>{x}</div>)}</div><div className="summary"><h3>Как использовать шаблон</h3><div className="summary-line"><b>Выбор</b><span>Откройте бота и выберите категорию документа.</span></div><div className="summary-line"><b>Заполнение</b><span>Внесите факты, даты, суммы и реквизиты сторон самостоятельно.</span></div><div className="summary-line"><b>Проверка</b><span>Если вопрос спорный или последствия значимы — передайте документ юристу или адвокату.</span></div><div className="summary-line"><b>Дальше</b><span>Для сложной ситуации можно перейти к платной AI-подготовке материалов.</span></div></div></div><div className="cta"><a className="btn primary" href={site.botUrl}>Открыть бесплатные образцы <span className="arrow">→</span></a><a className="btn ghost" href="/example">Посмотреть пример результата</a></div></div></section>

      <section className="result" id="analysis"><div className="wrap"><div className="section-title"><div><div className="kicker">AI-разбор</div><h2>AI-подготовка по вопросу или документу</h2></div><p>Сервис помогает увидеть структуру ситуации: какие факты важны, какие условия документа требуют внимания специалиста и что уточнить перед консультацией. Такое резюме облегчает дальнейшую работу адвоката.</p></div><div className="result-grid"><div className="result-card"><h3>Что входит</h3>{checks.map(x=><div className="check" key={x}>{x}</div>)}</div><div className="summary"><h3>Пример результата</h3><div className="summary-line"><b>Ситуация</b><span>Пользователь загрузил договор услуг и описал спор по качеству результата.</span></div><div className="summary-line"><b>Зоны внимания</b><span>Сервис выделил неоднозначные сроки, порядок приёмки и возврата оплаты — для проверки специалистом.</span></div><div className="summary-line"><b>Вопросы</b><span>Что уточнить у адвоката: доказательства недостатков, сроки претензии, перспективы возврата.</span></div><div className="summary-line"><b>Оговорка</b><span>Это предварительная подсветка, а не экспертная оценка, юридическое заключение или гарантия исхода.</span></div></div></div></div></section>
