...
const situations = [
  ["Вернуть деньги за товар или услугу", "Бот поможет собрать факты покупки, сроки, переписку и подготовить черновик претензии или список вопросов для юриста."],
  ["Работодатель задерживает выплату", "Сервис подскажет, какие даты, суммы, документы и обращения стоит собрать перед разговором со специалистом."],
  ["Контрагент не платит по договору", "Можно структурировать долг, условия договора, акты, переписку и подготовить основу для претензионного шага."],
  ["Жалоба в УК, банк или РКН", "Бесплатные шаблоны и предварительная сводка помогут оформить обращение понятнее и без лишних персональных данных."],
...
const pricingChoices = [
  ...
  ["Подписка", "Если регулярно готовите материалы, вопросы и черновики для дальнейшей проверки специалистом."],
];
...
{/* Modern Hero Section */}
      ...
      <section id="data-safety"><div className="wrap"><div className="section-title"><div><div className="kicker">Данные и безопасность</div><h2>Аккуратно с персональными данными</h2></div><p>Бот помогает подготовить материалы, но не просит паспортные данные, медицинские документы или лишние реквизиты без необходимости. По возможности описывайте ситуацию обезличенно.</p></div><div className="result-grid"><div className="result-card"><h3>Что важно помнить</h3><div className="check">не отправляйте паспорта, адреса, телефоны и банковские данные без необходимости</div><div className="check">заменяйте ФИО на роли: продавец, работодатель, контрагент, заявитель</div><div className="check">Удалить текущую заявку можно командой <code>/delete</code> в боте</div><div className="check">по вопросам обработки данных напишите в поддержку</div></div><div className="summary"><h3>Документы сервиса</h3><div className="summary-line"><b>Политика</b><span><a href="/privacy">/privacy</a> — как описана обработка данных.</span></div><div className="summary-line"><b>Согласие</b><span><a href="/consent">/consent</a> — условия обработки персональных данных.</span></div><div className="summary-line"><b>Условия</b><span><a href="/terms">/terms</a> — правила использования сервиса.</span></div><div className="summary-line"><b>Граница</b><span>Telegram и технические провайдеры участвуют в работе сервиса в объёме, необходимом для ответа.</span></div></div></div></div></section>
      ...
      <section id="pricing-choice"><div className="wrap"><div className="section-title"><div><div className="kicker">Выбор режима</div><h2>Что выбрать: шаблоны, пробный разбор или подписку</h2></div><p>Начните с бесплатного варианта и переходите к подписке только если регулярно готовите материалы.</p></div><div className="audiences">{pricingChoices.map(([title,text])=><article className="aud" key={title}><div className="mini">Вариант</div><h3>{title}</h3><p className="muted">{text}</p></article>)}</div></div></section>
